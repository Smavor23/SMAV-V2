...
//--------------------------------§URL SERVER§------------------------------
char *DEVEUI = "SMAV POWER REDUCE\n";        //DEVICE ID GLOBALE VARIABLE
const char *base_url = "http://app.smav-agro.com:8080/api/v1/vqMuDrXoF91Zc2vq434w/";
...
void StartDiagnosticTask(void const * argument)
{
  ...
  for(;;)
  {
    ...
    snprintf(data, sizeof(data), "{\"Wind_stat\":%.2f,\"Gas_lvl\":%.2f,\"Temp1\":%.2f,\"Temp2\":%.2f,\"Hum1\":%.2f,\"Hum2\":%.2f,\"rssi\": \"%s\",\"operatorName\": \"%s\",\"moduleStatus\": \"%s\",\"S_Batterie\":%.2f}",
             Wind_stat, Gas_lvl, temperature1, temperature2, humidity1, humidity2,rssiString,operatorName,Module_Status,VBattery);
